...
export function BuildingMilestonesLog({ milestones }: BuildingMilestonesLogProps) {
  if (milestones.length === 0) {
    return (
      <div className="mt-8 bg-white rounded-2xl border border-gray-200 shadow-sm p-10 text-center">
        ...
        <p className="text-gray-600 font-medium">No milestones recorded for this building yet.</p>
        <p className="text-sm text-gray-400 mt-1">Logged milestones will appear here in chronological order.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-8 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden animate-fade-in-up">
      <div className="p-6 sm:p-8 border-b border-gray-100 bg-gradient-to-b from-gray-50/50 to-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Chronological log of milestones for this building.</p>
          </div>
        </div>
      </div>
      ...
      <div className="relative py-4 px-4 sm:px-6">
        {/* Vertical line */}
        <div
          className="absolute left-[27px] sm:left-[31px] top-8 bottom-8 w-px bg-gradient-to-b from-[#134e4a]/25 via-[#134e4a]/15 to-transparent"
          aria-hidden
        />
          ...
          {milestones.map((m, index) => {
            ...
            return (
              <li key={m.id} className="relative flex gap-4 sm:gap-5">
                {/* Timeline dot */}
                <div className="relative z-10 shrink-0 w-14 sm:w-16 flex justify-center pt-5">
                  <div
                    className={`size-3 rounded-full border-2 border-white ${
                      isLast ? "bg-[#134e4a]" : "bg-[#134e4a]/80"
                    } ring-4 ring-[#134e4a]/10`}
                  />
                </div>
                {/* Content card */}
                <div className="flex-1 min-w-0 pb-8">
                  <div className="rounded-xl border border-gray-100 bg-gray-50/50 hover:bg-gray-50 hover:border-gray-200 transition-all duration-200 p-4 sm:p-5">
