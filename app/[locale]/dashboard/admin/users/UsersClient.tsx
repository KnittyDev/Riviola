...
function roleLabelKey(role: string): "roleAdmin" | "roleStaff" | "roleInvestor" {
  ...
  const filteredUsers = useMemo(() => {
    return initialUsers.filter((user) => {
      ...
      const matchesBuilding =
        buildingFilter === "all" || user.investor_properties?.some((p) => p.building_id === buildingFilter);
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-gray-500 font-medium font-inter">{t("users.subtitle")}</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="px-4 py-2 bg-orange-50 text-orange-600 rounded-2xl text-xs font-black uppercase tracking-widest border border-orange-100 flex items-center gap-2 shadow-sm shadow-orange-600/5 transition-transform hover:scale-105">
            ...
            {t("users.listed", { count: filteredUsers.length })}
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="relative group">
            <select
              ...
              className="w-full pl-12 pr-4 py-4 rounded-2xl border-2 border-gray-50 bg-gray-50/30 focus:border-[#134e4a] focus:bg-white outline-none font-black text-gray-900 appearance-none shadow-sm group-hover:bg-gray-50 uppercase text-[10px] tracking-widest"
            >
              <option value="all">{t("users.rolesAll")}</option>
              <option value="admin">{t("users.rolesAdmin")}</option>
              <option value="staff">{t("users.rolesStaff")}</option>
              <option value="investor">{t("users.rolesInvestor")}</option>
            </select>
          ...
          <div className="relative group">
            <select
              ...
              className="w-full pl-12 pr-4 py-4 rounded-2xl border-2 border-gray-50 bg-gray-50/30 focus:border-[#134e4a] focus:bg-white outline-none font-black text-gray-900 appearance-none shadow-sm group-hover:bg-gray-50 uppercase text-[10px] tracking-widest"
            >
              <option value="all">{t("users.buildingsAll")}</option>
              {buildings.map((b) => (
          ...
          <div className="relative group">
            <select
              ...
              className="w-full pl-12 pr-4 py-4 rounded-2xl border-2 border-gray-50 bg-gray-50/30 focus:border-[#134e4a] focus:bg-white outline-none font-black text-gray-900 appearance-none shadow-sm group-hover:bg-gray-50 uppercase text-[10px] tracking-widest"
            >
              <option value="all">{t("users.typeAny")}</option>
              <option value="buyer">{t("users.typeBuyer")}</option>
              <option value="renter">{t("users.typeRenter")}</option>
            </select>
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 pointer-events-none">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100 uppercase">
                <th className="px-8 py-5 text-[10px] font-black tracking-widest text-gray-400">{t("users.thUser")}</th>
                <th className="px-8 py-5 text-[10px] font-black tracking-widest text-gray-400">{t("users.thClassification")}</th>
                <th className="px-8 py-5 text-[10px] font-black tracking-widest text-gray-400">{t("users.thProjects")}</th>
                <th className="px-8 py-5 text-[10px] font-black tracking-widest text-gray-400 text-right">{t("users.thActions")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 font-inter">
              {filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-8 py-12 text-center text-gray-300 italic">
                    {t("users.empty")}
                  </td>
                </tr>
              ) : (
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col">
                            <span className="text-sm font-black text-gray-900 leading-tight mb-0.5 group-hover:text-orange-600 transition-colors">
                              {user.full_name || t("users.anonymous")}
                            </span>
                            <span className="text-xs text-gray-400 font-medium lowercase">{user.email}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                          ...
                          {user.role === "investor" ? (
                            investorCos.length > 0 ? (
                              investorCos.map((co, idx) => (
                                <span
                                  key={idx}
                                  className="bg-gray-50 text-gray-600 text-[10px] font-black px-3 py-1.5 rounded-xl uppercase tracking-tight border border-gray-100 flex items-center gap-2 group-hover:bg-white group-hover:border-orange-100 transition-all"
                                >
                              ...
                              <span className="text-gray-300 font-black italic text-xs tracking-tighter">{t("users.noPortfolios")}</span>
                            )
                          ) : (
                            <span className="text-sm font-bold text-gray-700 bg-gray-50 px-4 py-2 rounded-2xl border border-gray-100 group-hover:border-[#134e4a]/20 group-hover:text-[#134e4a] transition-all">
                              {user.companies?.name || t("users.riviolaHq")}
                            </span>
                          )}
                        </div>
                      </td>
