...
export function SettingsClient({ initialMaintenanceMode }: { initialMaintenanceMode: boolean }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div className={`bg-white p-10 rounded-[3.5rem] border-2 transition-all duration-500 overflow-hidden relative group ${
        ...
        <div className="relative z-10 flex flex-col items-center justify-center space-y-6">
           <div className={`size-20 rounded-3xl flex items-center justify-center transition-all duration-500 ${
           ...
           <div className="text-center">
              <h3 className="text-2xl font-black text-gray-900 tracking-tight mb-2">Maintenance Mode</h3>
              <p className="text-gray-400 text-sm font-medium font-inter max-w-[280px] leading-relaxed">
                 {maintenanceMode 
                   ? "Platform is currently UNDER MAINTENANCE. All non-admin triggers are disabled." 
                   : "Platform is LIVE. Standard protocols and user access are operational."}
              </p>
           </div>
           ...
           <button 
             ...
             className={`px-12 py-5 rounded-[2rem] font-black uppercase text-xs tracking-[0.2em] transition-all active:scale-95 shadow-xl ${
               maintenanceMode 
                 ...
                 : 'bg-[#134e4a] text-white shadow-[#134e4a]/20'
             } ${loading && 'opacity-50 pointer-events-none'}`}
           >
              {loading ? 'Propagating System Changes...' : (maintenanceMode ? 'Disable Maintenance' : 'Enable Maintenance')}
           </button>
           ...
           {maintenanceMode && (
              <div className="mt-4 flex items-center gap-2 px-4 py-2 bg-orange-50 rounded-full border border-orange-100">
                 ...
                 <span className="text-[10px] font-black text-orange-600 uppercase tracking-widest">Active Site Override</span>
              </div>
           )}
        </div>
        
        {/* Decorative background element */}
        <div className={`absolute top-0 right-0 w-40 h-40 rounded-full -translate-y-1/2 translate-x-1/2 transition-all duration-1000 ${
          maintenanceMode ? 'bg-orange-600/5' : 'bg-gray-50'
        }`} />
      </div>

      {/* Global Service Fees Card (Placeholder) */}
      <div className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-xl shadow-black/5 opacity-60 flex flex-col items-center justify-center space-y-6 grayscale relative overflow-hidden">
         ...
         <div className="text-center">
            <h3 className="text-2xl font-black text-gray-900 tracking-tight mb-2">Global Service Fees</h3>
            <p className="text-gray-400 text-sm font-medium font-inter max-w-[280px] leading-relaxed">
               Configure platform-wide transaction fees and Riviola subscription layers.
            </p>
         </div>
